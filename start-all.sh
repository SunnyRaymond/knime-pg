...
# ───► NEW — force postgres password = 123456  (runs each boot, harmless if unchanged)
gosu postgres psql -c "ALTER USER postgres PASSWORD '123456';" >/dev/null
...
(
  while true; do
    ...
    echo "[warn ] KNIME exited – restarting in 5 s …"
    sleep 5
  done
) &
KNIME_PID=$!
